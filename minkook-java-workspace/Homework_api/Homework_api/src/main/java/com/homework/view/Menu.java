	...
	public void stringGameMenu() {
		...
		while(true) {
			...
			if(menu == 1) {
				...
				System.out.print("문자열(String타입): ");
				String str = sc.nextLine();
				System.out.print("탐색할 문자(char): ");
				char ch= sc.nextLine().charAt(0);
				int count = calc.findCharCount(str,ch);
				System.out.println("해당문자는 총 " + count + "개입니다");
				...
				System.out.print("문자열(String타입): ");
				String str = sc.nextLine();
				int count = calc.selectTokenCount(str);
				if(str.contains("")){
					System.out.println("공백 포함해서 입력하라니깐요.");
				}else{
					System.out.println("공백기준으로 문자개수는 " + count + "입니다.");
				}
			}else if(menu == 3) {
				...
				System.out.print("문자열(String타입): ");
				String str = sc.nextLine();
				String res = calc.toSpaceUpper(str);
	...
	public void mathGameMenu() {
		...
		while(true) {
			...
			if(menu == 1) {
				...
				String  dNum1 = String.valueOf(Double.valueOf(num1)).toString();
				String  dNum2 = String.valueOf(Double.valueOf(num2)).toString();
				//System.out.println(dNum1);
				//System.out.println(dNum2);
				//System.out.println(num1.equals(dNum1));
				int res = 0;
				if(num1.equals(dNum1) && num2.equals(dNum2)){
					dNum1 = String.valueOf(Double.valueOf(num1).intValue());
					dNum2 = String.valueOf(Double.valueOf(num2).intValue());
					res = calc.sumString(dNum1,dNum2);
					System.out.println(res);

				}else{
					res = calc.sumString(num1,num2);
					System.out.println(res);
				}
				...
				if(num1.equals(dNum1) && num2.equals(dNum2)){
					...
					res = calc.minusString(num1,num2);
					System.out.println(res);
				}
	...
	public void dateGameMenu() {
		...
		while(true) {
			...
			if(menu == 1) {
				...
				System.out.print("일");
				String dayofMonth = sc.nextLine();

				calc.makeCalendar(year,month,dayofMonth);

				calc.printFormat(calc.makeCalendar(year,month,dayofMonth));
				...
				System.out.print("년도: ");
				String year = sc.nextLine();
				boolean res = calc.isLeapYear(Integer.parseInt(year));
				if(res == true){
				...
				int iFirstYear = Integer.parseInt(firstYear);
				int iEndYear = Integer.parseInt(endYear);

				if(iFirstYear > iEndYear){
					System.out.println("시작년도를 더 작은 숫자로 써야지..");
				}else {
					Long res = calc.leapDate(iFirstYear,iEndYear);
					System.out.println("총 일수는 " + res+ "일입니다.");
				}

...
import com.younggalee.practice1.dto.Book;
...
public class BookController {
    ...
    public void insertBook(String newTitle, String newAuthor, String newPublisher, String newDate, String newPrice) {
        ...
        Calendar dateData = new GregorianCalendar(Integer.parseInt(stn.nextToken()), Integer.parseInt(stn.nextToken()), Integer.parseInt(stn.nextToken()));

        list[list.length-1] = new Book(newTitle, newAuthor, newPublisher, dateData, Integer.parseInt(newPrice));

    }

    public void printBookPublishDate() {

        Calendar finalBooKDate = list[4].getPublishDate();
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy년 MM월 dd일");

        System.out.println(sdf.format(finalBooKDate.getTimeInMillis()));
//        String dateStr = sdf.format(today);
//
//        int year = finalBooKDate.set(Calendar.YEAR, );
//        int month = finalBooKDate.get(Calendar.MONTH);
//        int date = finalBooKDate.get(Calendar.DATE);
//        System.out.printf("%d년 %d월 %d일 출간", year,month,date);

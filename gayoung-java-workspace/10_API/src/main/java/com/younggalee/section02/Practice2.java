...
import com.younggalee.section02.dto.Shop;
...
public class Practice2 {
    public static void main(String[] args) {
        ...
        String[] shop = shopCsv.split("\n");

        Shop[] shopArr = new Shop[10];
        for (int i=0; i<shopArr.length; i++){
            String[] shopInfo = shop[i].split(",");
            int index = Integer.parseInt(shopInfo[0]);
            String name = shopInfo[1];
            String address = shopInfo[2];
            String phone = shopInfo[3];
            String kind = shopInfo[4];
            String date = shopInfo[5];

            shopArr[i] = new Shop(index, name, address, phone, kind, date);

	...
	public void stringGameMenu() {
		...
		while(true) {
			...
			if(menu == 1) {
				...
				System.out.println("탐색할 문자를 입력하세요.");
				char c = sc.next().charAt(0);
				int cnt = calc.findCharCount(str, c);
				...
				if(!str.contains(" ")){
					...
					int cnt = calc.selectTokenCount(str);
					System.out.println(cnt);
				}
				...
				System.out.println("문자열을 입력하세요.");
				String str = sc.nextLine();
				String s = calc.toSpaceUpper(str);
				...
				FoodShop[] food = calc.csvFormat();
				for(FoodShop foodShop : food) {
					System.out.println(foodShop.toString());
				}
	...
	public void mathGameMenu() {
		...
		while(true) {
			...
			if(menu == 1) {
				...
				int num = calc.sumString(str, str1);
				...
				if(str.contains(".") || str1.contains(".")) {
					...
					int num = calc.minusString(str, str1);
					System.out.println(num);
				}
	...
	public void dateGameMenu() {
		...
		while(true) {
			...
			if(menu == 1) {
				...
				Calendar c = calc.makeCalendar(str,str1,str2);
				calc.printFormat(c);
				...
				sc.nextLine();
				System.out.println("끝 년도 : ");
				int end = sc.nextInt();
				sc.nextLine();
				if(start > end) {
					System.out.println("시작년도를 더 작은 숫자로 써야지..");
				}else {
					long result = calc.leapDate(start, end);

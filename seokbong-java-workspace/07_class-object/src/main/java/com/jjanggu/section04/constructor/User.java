package com.jjanggu.section04.constructor;
...
public class User {
    ...
    /*
        ...
        5. 생성자의 종류
           1) 기본 생성자
              - 매개변수가 없기 때문에 생성자 호출시 별도의 전달값 받을 수 없음
           ...
           2) 매개변수 생성자
              - 매개병수 있는 생성자로, 생성자 호출시 전달값을받아서 사용할 수 있음
              - 주로 전달된 값을 필드에 초기화 할 목적로 사용
                즉, 갹체 생성과 동시에 필드에 원하는 값을 초기화할 때 주로 사용
              - 작성형식
                접근제어자 생성자명(매개변수, ...){
                    this.필드 = 매개변수;
                    this.필드 = 매개변수;
    ...
    public User(String id, String pwd, String name){
        ...
        this.pwd = pwd;
        this.name = name;
        System.out.println("User 클래스의 모든 필드(id, pwd, name)를 초기화하는 생성자 호출됨..");
    }
    */
    public User(String id, String pwd, String name){
        this(id,pwd); // 사전에 작성되어 있는 다른 생성잘를 호출
        this.name = name;
        System.out.println("User 클래스의 모든 필드(id, pwd, name)를 초기화하는 생성자 호출됨..");

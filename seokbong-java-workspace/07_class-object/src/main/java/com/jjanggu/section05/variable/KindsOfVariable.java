package com.jjanggu.section05.variable;
...
public class KindsOfVariable {
    ...
    /*
        1. 인스턴스 변수
           1) 선언위치 : class block
           2) 생성시기 : 해당 클래스를 통해서 객체생성시(인스턴스화) Heap 메모리에 할당 (JVM 자동초기화)
           3) 소멸시기 : 인스턴스 소멸시
           4) 사용범위 : 클래스 전역
     */
    ...
    public void test(int paramNum){
        /*
            3. 지역 변수 (매개변수도 해당됨)
               ...
               3) 소멸시기 : 해당 block이 종료시
               4) 사용범위 : 선언된 해당 block
         */
        int localNum = 200;

        System.out.println("매개변수: " + paramNum);
        System.out.println("지역변수: " + localNum); // 지역변수는 초기화(변수에 수를 할당)하지 않으면 사용불가

...
//        arr = new int[3];// 배열 생성
    ...
    public void testBasicArray2() {
        ...
        // 각 인덱스 자리의 배열요소에 이미 데이터가 담겨있음 Heap메모리가 가지는 특징 때문
        // 힙메모리는 빈 공간이 존재 할 수 없기 때문에 기본값이 들어간 배열이 생성
...
//        arr = {1, 2, 3, 4} // 오류
        ...
        System.out.println(arr1 == arr2); // false 참조변수는 주소값으로 비교되기 때문
...
//            System.out.println("arr[" + i + "] : " + arr[i]);
    ...
    public void testArrayExample3() {
        /*
            사용자에게 배열의 길이 입력받아 해당길이만큼의 문자열 배열
    ...
    public void testArrayExample4() {
        ...
        System.out.println(arr); // char[] 참조변수는 출력시 각 요소들을 하나으 문자열로 합쳐서 반환되어 출력
    }
    ...
    public void testArrayFault() {
        ...
        System.out.println("arr의 주소 : " + arr.hashCode());
...
//        arr[5] = 12;
        ...
        arr = new int[7]; // 새로운 곳 참조(기존 배열은 끊김)

        System.out.println("변경된 배열의 주소값 : " + arr.hashCode());

        /*
            연결이 끊겨진 Heap영역에서의 기전 배열 객체는 일정시간 후에 GC(가비지 컬렉터)가 삭제시킴
            => 자바에서의 "자동메모리 관리" 특징
            => 개발자는 메모리 관리 신경쓸 필요 없이 개발에만 집중할 수 있음
         */

        // 만일 현재 배열 객체 마저도 제거시키고자 할 경우 => 연결을 끊으면 됨 => null 대입
        arr = null;
    }
    ...
    public void testEnhancedFor() {
        /*
            ...
            5. JDK1.5(ver.5) 이상부터 사용 가능
            6. 작성 형식
                 for(변수선언 : 순회할 배열 및 컬렉션) {
                     변수활용
                 }
         */
        ...
        // 향상된 for문 내의 변수 탕비은 배열 요소 타입으로 작성하면됨

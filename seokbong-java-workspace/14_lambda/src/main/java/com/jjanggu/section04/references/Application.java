package com.jjanggu.section04.references;
...
public class Application {
    public static void main(String[] args) {
        /*
            ...
            3. 함수형 인터페이스의 매개변수, 반환타입과 참조할 메소드으의 매개변수, 반환타입이 같아야됨
            4. 표현식
               1) 정적 메소드 참조           => 클래스명 :: staticMethodName
               2) 인스턴스 메소드참조         => 참조변수 :: instanceMethodName
               3) 특정 타입의 객체 메소드 참조 => 타입:: methodName
               4) 생성자 참조                => 클래스명 :: new
         */

        // case1. 특정 정수값을 전달받아 절대값을 반환해주는 Function
//        Function<Integer,Integer> function = num -> Math.abs(num); // 전달값 가지고 static 메소드 싱행 결과 반환                                                                                                                    >
        Function<Integer, Integer> function = Math:: abs;
        System.out.println(function.apply(-5));

        // case2. 특정값을 전달받아 단지 출력만해주는 Consumer
        // Consumer<Object> consumer = obj -> System.out,println(obi); // 전달값 가지고 System.out(PrintStream 인스턴스)의 println 메소드 실행
        ...
        // * forEach(Consumer<T> consumer) : 해당 메소드 호출시 순차적으로 요소를 가지고 어떤내용을 수행시킬건지의 람다식 전달

...
import com.inyong.section02.dto.Shop;
...
public class Practice2 {
    public static void main(String[] args) {
        ...
        String[] shopInfo = shopCsv.split("\n");

        Shop[] shops = new Shop[10];

        for(int i = 0; i < shopInfo.length; i++){

            String[] infoArr = shopInfo[i].split(",");

            String id = infoArr[0];
            String name = infoArr[1];
            String address = infoArr[2];
            String phone = infoArr[3];
            String category = infoArr[4];
            String registeredDate = infoArr[5];

            shops[i] = new Shop(id, name, address, phone, category, registeredDate);
        }
